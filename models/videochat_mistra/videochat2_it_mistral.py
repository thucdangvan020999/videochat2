...
class VideoChat2_it_mistral(Blip2Base):
    ...
    def __init__(self, config):
        ...
        self.vision_encoder, self.vision_layernorm = (
        ...
        if extra_num_query_token > 0:
            ...
            self.extra_query_tokens = nn.Parameter(
                torch.zeros(
                    1, extra_num_query_token, self.query_tokens.shape[-1]
                )
            )
        ...
        if self.debug:
            ...
            self.mistral_model = AutoModelForCausalLM.from_config(
                mistral_config
            )
        else:
        ...
        self.mistral_proj = nn.Linear(
            ...
            self.mistral_model.config.hidden_size,
        )
    ...
    def encode_img(self, image, instruction):
        ...
        with self.maybe_autocast():
            ...
            image_embeds = self.vision_layernorm(image_embeds).to(
                device
            )  # [B, T*L, C]
    ...
    def forward(self, image, text_input, instruction):
        ...
        for idx, prompt in enumerate(text_input):
            ...
            if self.use_lora:
                p_before_embeds = (
                    self.mistral_model.base_model.model.model.embed_tokens(
                ...
                p_before_embeds = self.mistral_model.model.embed_tokens(
                    p_before_tokens.input_ids
                )
                p_after_embeds = self.mistral_model.model.embed_tokens(
                    p_after_tokens.input_ids
                )
            ...
            for text in raw_text[1:-1]:
                ...
                answer_targets[
                    :, (cur_len + ans_len) : (cur_len + total_len)
                ] = -100
                cur_len += total_len
            ...
            if self.debug:  # Inspect and check the correctness of masking
                ...
                z = torch.where(
                    z == -100, self.mistral_tokenizer.unk_token_id, z
                )
        ...
        inputs_embeds = (
            torch.ones([batch_size, txt_len], dtype=torch.long).to(
                img_embeds.device
            )
            * self.mistral_tokenizer.pad_token_id
        )
        if self.use_lora:
            inputs_embeds = (
                self.mistral_model.base_model.model.model.embed_tokens(
        ...
        attention_mask = torch.zeros(
            [batch_size, txt_len], dtype=torch.long
        ...
        for idx in range(batch_size):
            ...
            targets[idx, (p_before_len + img_len + 1) : (input_len + 1)] = (
                target_list[idx][0, : (input_len - p_before_len - img_len)]
            )
